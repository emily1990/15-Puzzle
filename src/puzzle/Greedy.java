...
public class Greedy {
	...
	public Greedy(final State start, final State goal,String string){
	}
	...
	//	Boolean flag = false;
		...
		while(open.size()>0){
			if(a==1000){
				System.out.println("Please wait...this is hard for Greedy(Manhattan)");
					
			}
			if(a>25000){
				if(string == "Man")
				{System.out.println("Greedy (Manhattan) cannot solve the problem in less than 25000 steps");
			}
				else{
					System.out.println("Greedy (Myheuristic) cannot solve the problem in less than 25000 steps");
